...
const Card = () => {
  let { customerBasket, removeFromBasket, calcBasket } = useContext(
    BasketContext
  );
    ...
    <div className={styles.cardWrapperContainer}>
      <div className={styles.cardWrapper}>
        <h2 className={styles.cardHeading}>
          Current {customerBasket.length === 1 ? "item" : "items"} in your
          basket
        </h2>
        ...
        <div>
          <p className={styles.float}>
            <span className={styles.fat}>Total price: </span>${totalPrice}
          </p>
        </div>
      </div>
    </div>
